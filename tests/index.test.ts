...
import { APIPromise } from 'canva/core/api-promise';

import util from 'node:util';
import Canva from 'canva';
import { APIUserAbortError } from 'canva';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['CANVA_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Canva) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Canva({ username: 'My Username', password: 'My Password' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['CANVA_LOG'] = 'debug';
      const client = new Canva({ logger: logger, username: 'My Username', password: 'My Password' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['CANVA_LOG'] = 'not a log level';
      const client = new Canva({ logger: logger, username: 'My Username', password: 'My Password' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'CANVA_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['CANVA_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['CANVA_BASE_URL'] = 'https://example.com/from_env';
      const client = new Canva({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['CANVA_BASE_URL'] = ''; // empty
      const client = new Canva({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://api.canva.com/rest');
    });

    test('blank env variable', () => {
      process.env['CANVA_BASE_URL'] = '  '; // blank
      const client = new Canva({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://api.canva.com/rest');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['CANVA_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Canva({ username: 'My Username', password: 'My Password' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Canva({ maxRetries: 4, username: 'My Username', password: 'My Password' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Canva({ username: 'My Username', password: 'My Password' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['CANVA_USERNAME'] = 'My Username';
    process.env['CANVA_PASSWORD'] = 'My Password';
    const client = new Canva();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['CANVA_USERNAME'] = 'another My Username';
    process.env['CANVA_PASSWORD'] = 'another My Password';
    const client = new Canva({ username: 'My Username', password: 'My Password' });
    expect(client.username).toBe('My Username');
    expect(client.password).toBe('My Password');
  });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Canva({ username: 'My Username', password: 'My Password', fetch: testFetch });
