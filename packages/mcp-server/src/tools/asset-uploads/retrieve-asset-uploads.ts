...
export const handler = async (client: Canva, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.assetUploads.retrieve(jobId)));
};
