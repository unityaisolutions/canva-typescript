...
import { AppRetrieveJwksResponse, Apps } from './resources/apps';
import {
...
import {
  ...
  OwnershipType,
  SortByType,
} from './resources/brand-templates';
import {
  CommentCreateReplyParams,
  CommentCreateReplyResponse,
  CommentCreateThreadParams,
...
import { Connect, ConnectRetrieveKeysResponse } from './resources/connect';
import {
  ExportCreateParams,
...
import {
  ...
  Resizes,
} from './resources/resizes';
import {
  URLAssetUploadCreateParams,
  URLAssetUploadCreateResponse,
  URLAssetUploadRetrieveResponse,
  URLAssetUploads,
} from './resources/url-asset-uploads';
import {
  URLImportCreateParams,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['CANVA_USERNAME'].
   */
  username?: string | null | undefined;

  /**
   * Defaults to process.env['CANVA_PASSWORD'].
   */
   ...
   *
   * Defaults to process.env['CANVA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CANVA_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Canva API.
 */
export class Canva {
   ...
   * API Client for interfacing with the Canva API.
   *
   * @param {string | null | undefined} [opts.username=process.env['CANVA_USERNAME'] ?? null]
   * @param {string | null | undefined} [opts.password=process.env['CANVA_PASSWORD'] ?? null]
   * @param {string} [opts.baseURL=process.env['CANVA_BASE_URL'] ?? https://api.canva.com/rest] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CANVA_BASE_URL'),
    username = readEnv('CANVA_USERNAME') ?? null,
    password = readEnv('CANVA_PASSWORD') ?? null,
    ...opts
  }: ClientOptions = {}) {
    const options: ClientOptions = {
      ...
      password,
      ...opts,
      baseURL: baseURL || `https://api.canva.com/rest`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Canva.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CANVA_LOG'), "process.env['CANVA_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.canva.com/rest';
  }
  ...
  static CanvaError = Errors.CanvaError;
  ...
  assetUploads: API.AssetUploads = new API.AssetUploads(this);
  urlAssetUploads: API.URLAssetUploads = new API.URLAssetUploads(this);
  autofills: API.Autofills = new API.Autofills(this);
  brandTemplates: API.BrandTemplates = new API.BrandTemplates(this);
  comments: API.Comments = new API.Comments(this);
  designs: API.Designs = new API.Designs(this);
  connect: API.Connect = new API.Connect(this);
  imports: API.Imports = new API.Imports(this);
  urlImports: API.URLImports = new API.URLImports(this);
  exports: API.Exports = new API.Exports(this);
  folders: API.Folders = new API.Folders(this);
  oauth: API.OAuth = new API.OAuth(this);
  resizes: API.Resizes = new API.Resizes(this);
  users: API.Users = new API.Users(this);
}

Canva.Apps = Apps;
Canva.Assets = Assets;
Canva.AssetUploads = AssetUploads;
Canva.URLAssetUploads = URLAssetUploads;
Canva.Autofills = Autofills;
Canva.BrandTemplates = BrandTemplates;
Canva.Comments = Comments;
Canva.Designs = Designs;
Canva.Connect = Connect;
Canva.Imports = Imports;
Canva.URLImports = URLImports;
Canva.Exports = Exports;
Canva.Folders = Folders;
Canva.OAuth = OAuth;
Canva.Resizes = Resizes;
Canva.Users = Users;

export declare namespace Canva {
  export type RequestOptions = Opts.RequestOptions;

  export { Apps as Apps, type AppRetrieveJwksResponse as AppRetrieveJwksResponse };
  ...
  export {
    URLAssetUploads as URLAssetUploads,
  ...
  export {
    BrandTemplates as BrandTemplates,
  ...
  export { Connect as Connect, type ConnectRetrieveKeysResponse as ConnectRetrieveKeysResponse };
